...
exports.validateAndAutoExtractResponseMleKid = function(merchantConfig, logger) {
    ...
    if (hasValidFilePath) {
        ...
        if (isP12File) {
            ...
            const isCybersourceP12 = exports.isCybersourceP12(
            ...
            if (isCybersourceP12) {
                ...
                try {
                    const extractedKid = exports.extractResponseMleKid(
                        merchantConfig.getResponseMlePrivateKeyFilePath(),
                        merchantConfig.getResponseMlePrivateKeyFilePassword(),
    ...
    // If auto-extraction didn't work, check if responseMleKID is manually configured
    let responseMleKID = merchantConfig.getResponseMleKID();
    if (responseMleKID && typeof responseMleKID === "string" && responseMleKID.trim()) {
        logger.debug('Using manually configured responseMleKID');
        return responseMleKID;
    }
