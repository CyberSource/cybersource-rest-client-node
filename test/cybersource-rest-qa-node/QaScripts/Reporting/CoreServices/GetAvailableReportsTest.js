...
function loadFileData() {
    ...
    var callback = function (error, data, response) {      
     ...
     var csvStream = csv.fromPath(path.join('QaScripts','CSV_Files','Reporting','CoreServices','RetrieveAvailableReports.csv'),
     { headers: true });
 ...
 csvStream.on("data", function (data) {      
    ...
    var timeQueryType = data['tQueryType'];
    var opts = [];
    opts['organizationId'] = 'testrest';

    api.searchReports(startTime, endTime, timeQueryType, opts, callback);
    while (sync) {           
...
csvStream.on("end", function () {
     TestRun.WriteOutData(resArr, testId, 'GetAvailableReports',assertMessage);
});
