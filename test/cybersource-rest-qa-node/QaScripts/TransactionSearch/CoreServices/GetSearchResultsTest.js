...
function loadFileData() {
    ...
    var callback = function (error, data, response) { 
        ...
        if (error) {
            ...
            assertMessage[count] = response.res.statusMessage;            
            count++;
        } else {
            try {          
                // Testing- Change Assertion  
                assert.equal(inputData['id'], response.body.searchId);
    ...
    var csvStream = csv.fromPath(path.join('QaScripts','CSV_Files','TransactionSearch','CoreServices','GetSearchResults.csv'),
    { headers: true });
...
csvStream.on("end", function () {
    ...
    TestRun.WriteOutData(resArr, testId, 'GetSearchResults',assertMessage);
});
