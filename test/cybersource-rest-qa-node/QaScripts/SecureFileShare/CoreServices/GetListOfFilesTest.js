...
function loadFileData() {
	...
	api = new cybersourceRestApi.SecureFileShareApi(configObject);    
    ...
    var csvStream = csv.fromPath(path.join('QaScripts','CSV_Files','SecureFileShare','CoreServices','GetListOfFiles.csv'),
        { headers: true });
...
csvStream.on("data", function (data) {      
    ...
    api.getFileDetails(startDate, endDate, opts, callback);
    while (sync) {           
...
csvStream.on("end", function () {
    TestRun.WriteOutData(resArr, testId, 'GetListOfFiles',assertMessage);
});
